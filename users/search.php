<?php 
    ...
    if($_SESSION['user_gender'] == 'male'){
        $searchFor = "AND pu.gender = 'female'";
    } else {
        $searchFor = "AND pu.gender = 'male'";
    }
    ...
    $allowInterCast = isset($_REQUEST['allowInterCast']) && $_REQUEST['allowInterCast'] == 'on' ? '1' : "0"; 
    $profileId = isset($_REQUEST['profileId']) ? $_REQUEST['profileId'] : ""; 
    
    // search filter for samaj
    if(isset($_GET['samaj'])){
        $filter_Samaj = "AND pu.samajId = '$_GET[samaj]' AND (pu.isReadyForInterCaste = '1' OR pu.samajId = '$_SESSION[user_samaj_id]')";
    } else {
        // $filter_Samaj = "AND (pu.isReadyForInterCaste = '1' OR pu.samajId = '$_SESSION[user_samaj_id]')";
        if($_SESSION['isReadyForInterCaste'] == '1' && $allowInterCast == '1'){
            $filter_Samaj = "AND (pu.isReadyForInterCaste = '1' OR pu.samajId = '$_SESSION[user_samaj_id]')";
        }
        else {
            $filter_Samaj = "AND (pu.samajId = '$_SESSION[user_samaj_id]')";
        }
    }
    ...
    // if($allowInterCast != ""){
    //     $searchFor .= " AND pu.isReadyForInterCaste = '%$allowInterCast%'";
    // }
    if($profileId != ""){
                                      ...
                                      <div id="AdvanceSearch" class="collapse" aria-labelledby="headingOne" data-parent="#SearchOptionsAccordion">
                                        <div class="card-body">
                                            <form action="#">
                                                ...
                                                <div class="form-group form-row">
                                                    <label class="col-md-2 form-check-label" for="allowInterCast">
                                                    आंतरजातिय प्रोफाइल
                                                    </label>
                                                    <div class="col-md-3">
                                                        <?php echo $allowInterCast; ?>
                                                    <input  type="checkbox" style="width: 40px;height: 20px;" id="allowInterCast" name="allowInterCast"  <?php if($allowInterCast == '1') 
        ...
        foreach($result->fetchAll() as $row) {
            ...
            <div class='profile-search-card ts-box row '>
                <div class='col-md-3 profile-img'>
                    ...
                    <img src='$row[pic1]' class='image--cover' />
                </div>
                <div class='col-md-9 profile-body pl-4'>
                    <h4><b class='mr-4'>$row[firstName], $row[age] </b> $row[currentCity], $row[currentState]</h4>
                    <h4>
                    ...
                    $sql1 = "SELECT  * FROM usersProfileAccess
                            WHERE profileUserId = $_SESSION[user_id] AND accessToProfile = $row[profileUserId] AND isDeleted = 0";
                            // die($sql1);
                    $result1 = $conn -> db -> query($sql1);
                    
                    if($result1->rowCount() <= 0){
                        echo "<button type='button' data-toggle='modal' data-target='#addToCart' class='btn btn-default icon-btns mr-2' data-toggle='tooltip' data-placement='top' title='Add to Cart' onClick='setCartProfileSession($row[profileUserId])'>
                            <i class='fa fa-plus' aria-hidden='true' ></i>
                        </button>";    
                    } else {
                        echo "<form method='POST' action='view-profile.php' target='_blank'>
...
<div class="modal fade" id="addToCart" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row no-gutters ts-cards-same-height">
            ...
            <div class="col-md-12">
                <p class="text-danger">वर्षभरासाठी अनलिमिटेड प्रोफाईल बघण्यासाठी वार्षिक वर्गणी पंधराशे अदा करून आपण गोल्डन मेंबरशिप चा लाभ घेऊ शकता किंवा प्रतेकि 20 रुपये भरून प्रोफाइल ची माहिती घेऊ शकतात
                </p>
            </div>            
      ...
      <div class="modal-footer">
        <a href="checkout.php" class="btn btn-sm btn-light">Checkout</a>
    ...
    <script type="text/javascript" >
        function setCartProfileSession(profileId){
            $.ajax({
                url:"ajax.php", //the page containing php script
                type: "post", //request type,
                dataType: 'json',
                data: { action: "setCartProfileSession", profileId: ""+profileId },
                success:function(result){
                    result.op.split("###");
                    $('#cartProfileSession').html(result.op.split("###")[0]); 
                    $('#cartButton').html(result.op.split("###")[1]); 
        ...
        function addToCartSession(val){
            ...
            $.ajax({
                ...
                type: "post", //request type,
                dataType: 'json',
                data: { action: "addToCartSession", profileId: ""+profileId },
                success:function(result){
